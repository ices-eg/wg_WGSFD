...
for(year in yearsToSubmit){
  ...
  tacsat_name <-
    load(
      file.path(
        dataPath,
        paste0("cleanTacsat_", year, ".RData")   # changed to German input data, i.e. clean Version already (internationl eflalo format)
      )); #- data is saved as tacsat_2009, tacsat_2010 etc
  eflalo_name <-
    load(
      file.path(
        dataPath,
        paste0("cleanEflalo_", year, ".RData")  # changed to German input data, i.e. clean Version already (internationl eflalo format)
  ...
  rm(tacsatp) # remove tacsatp since this name is reused in code 2_eflalo....
  #-------------------------------------------------------------------------------
  ...
  save(
    remrecsEflalo,
    file = file.path(outPath, paste0("remrecsEflalo", year, ".RData", sep=""))
  )
