...
ve <- 
  ...
  read_ve("/media/fssf01bhv/vmsdata/DataGlobal/deu2ices/Hackathon_Test_20220215/table1Save.csv") %>% 
  csq2pos() %>% 
...
le <- 
  #read_le("/media/fssf01bhv/vmsdata/DataGlobal/deu2ices/2020/table2_DE_v20210505_181147_2009-2020.csv")
  read_le("/media/fssf01bhv/vmsdata/DataGlobal/deu2ices/Hackathon_Test_20220215/table2Save.csv")
...
#### Countries in table 1 (vms, ve):  `r as.character(unique(ve$country))`
#### Countries in table 2 (log, le):  `r as.character(unique(le$country))`
...
VEGear <- left_join(ve, GearType, by = c("dcf4" = "Key"))%>%
  select(dcf4, VocabGear) %>%
  distinct()
...
VETargetAssemblage <- left_join(ve, TargetAssemblage, by = c("dcf5" = "Key"))%>%
  select(dcf5, VocabTargetAssemblage) %>%
  distinct()
...
LEGear <- left_join(le, GearType, by = c("dcf4" = "Key"))%>%
  select(dcf4, VocabGear) %>%
  distinct()
...
LETargetAssemblage <- left_join(le, TargetAssemblage, by = c("dcf5" = "Key"))%>%
  select(dcf5, VocabTargetAssemblage) %>%
  distinct()
