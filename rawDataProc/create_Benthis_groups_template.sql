...
FROM (
  SELECT  Data.year, 
          ...
          SUM(Data.fishing_hours * isnull(([FirstFactor]*Power(avg_kw,[SecondFactor]))/cast(1000 as float), B.gearWidth) * Data.ICES_avg_fishing_speed * 1.852) AS Surface,  
          JNCC.Benthis_metiers, 
          subsurface_prop 
  FROM  tblAux_benthisGearWidthsForWGSFD15 AS B RIGHT OUTER JOIN
          tblAux_Lookup_Metiers_incl_log AS JNCC ON B.benthis_met = JNCC.Benthis_metiers RIGHT OUTER JOIN
            %1$s AS Data ON JNCC.LE_MET_level6 = LTRIM(RTRIM(REPLACE(Data.LE_MET_level6, '"', '')))
  WHERE  B.benthis_met in ('OT_CRU', 'OT_DMF', 'OT_MIX', 'OT_MIX_CRU', 'TBB_CRU', 'TBB_DMF')
  GROUP BY Data.year, Data.c_square, JNCC.Benthis_metiers,	subsurface_prop
  UNION ALL
  SELECT  Data.year, 
          Data.c_square,  
          COUNT(*) AS [No. Recs], 
          ...
          SUM(Data.kw_fishinghours) AS kw_fishinghours, 
          SUM(Data.fishing_hours) AS fishing_hours, 
          SUM(Data.fishing_hours * isnull(([FirstFactor]*avg_oal + [SecondFactor])/cast(1000 as float),B.gearWidth) * Data.ICES_avg_fishing_speed * 1.852) AS Surface,  
          JNCC.Benthis_metiers, 
          subsurface_prop	 
  FROM  tblAux_benthisGearWidthsForWGSFD15 AS B RIGHT OUTER JOIN
          tblAux_Lookup_Metiers_incl_log AS JNCC ON B.benthis_met = JNCC.Benthis_metiers RIGHT OUTER JOIN
  ...
  SELECT  Data.year, 
          ...
          SUM(Data.kw_fishinghours) AS kw_fishinghours, 
          SUM(Data.fishing_hours) AS fishing_hours, 
          SUM(Data.fishing_hours * isnull(([FirstFactor]*Power(avg_oal,[SecondFactor]))/cast(1000 as float),B.gearWidth) * Data.ICES_avg_fishing_speed * 1.852) AS Surface, 
          JNCC.Benthis_metiers,
          subsurface_prop	 
  FROM  tblAux_benthisGearWidthsForWGSFD15 AS B RIGHT OUTER JOIN
          tblAux_Lookup_Metiers_incl_log AS JNCC ON B.benthis_met = JNCC.Benthis_metiers RIGHT OUTER JOIN
...
GROUP BY year, c_square, Benthis_metiers
