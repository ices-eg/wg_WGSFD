...
#Read in latest submission -->
ICES_LE <- read.csv('data/ICES_LE_BEL.csv', stringsAsFactors=FALSE, na.strings = 'NULL')
ICES_VE <- read.csv('data/ICES_VE_BEL.csv', stringsAsFactors=FALSE, na.strings = 'NULL')
# fix erroneous types
...
spatialplot <- 
  ggplot(polLand, aes(long, lat)) +
    ...
    coord_fixed(ratio = vmstools::lonLatRatio(mean(spatBound$xrange), mean(spatBound$yrange)),
...
# origional code: 190 secs for Belgium.
#system.time({
...
ib_by <- as.integer(as.factor(b_by, levels = a_by))
polVMS$cols <- dat$cols[ib_by]

# do plots
lapply(top3gears, function(gear) {
spatialplot +
  geom_polygon(data = polVMS[polVMS$gear_code == gear,], 
...
dat2tab <- 
  with(ICES_VE, 
       tapply(totweight, list(year, gear_code), sum, na.rm = TRUE))
names(dimnames(dat2tab)) <- c("year","gear_code")
kable(dat2tab)
...
dat2plot$ratio <- 
  with(dat2plot,
       ifelse(fishing_hours == 0, 9,
         ifelse(fishing_hours_median == 0, -9,
           ifelse(fishing_hours_median == 0 & fishing_hours_median == 0, 0,      
             log10(fishing_hours_median) - log10(fishing_hours)))))
...
dat2tab <- 
  with(ICES_VE, 
       tapply(totweight/kw_fishinghours, list(year, gear_code), median, na.rm = TRUE))
names(dimnames(dat2tab)) <- c("year","gear_code")
kable(dat2tab)
...
dat2tab <- 
  with(ICES_VE, 
       tapply(totvalue, list(year, gear_code), sum, na.rm = TRUE))
names(dimnames(dat2tab)) <- c("year","gear_code")
kable(dat2tab)
...
dat2tab <- 
  with(ICES_VE, 
       tapply(totvalue/kw_fishinghours, list(year, gear_code), median, na.rm = TRUE))
names(dimnames(dat2tab)) <- c("year","gear_code")
kable(dat2tab)
...
dat2tab <- 
  with(ICES_VE, 
       tapply(totvalue/totweight, list(year, gear_code), median, na.rm = TRUE))
names(dimnames(dat2tab)) <- c("year","gear_code")
kable(dat2tab)
