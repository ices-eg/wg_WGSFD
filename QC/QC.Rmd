...
if(compareSubmission){
  ...
  fishing_days_sum_LEP    <- round(sum(ICES_LEP$fishing_days,na.rm=TRUE), digits=1)
  kw_fishing_days_sum_LEP <- round(sum(ICES_LEP$kw_fishing_days, na.rm=TRUE), digits=1)
  totweight_min_LEP       <- round(min(ICES_LEP$totweight,na.rm=TRUE), digits=1)
  totweight_max_LEP       <- round(max(ICES_LEP$totweight,na.rm=TRUE), digits=1)
...
```{r area-of-data, fig.width = 5.5, fig.height = 9}
...
```{r spatial-extent-dominant-gears, results='asis',fig.width = 5.5, fig.height = 9}
...
```{r spatial-extent-dominant-gears2, results='asis',fig.width = 5.5, fig.height = 9}
ggplot(polLand,aes(long,lat)) +
  geom_polygon(aes(group=group)) +
  coord_fixed(ratio=lonLatRatio(mean(spatCore$xrange),mean(spatCore$yrange)),xlim=spatCore$xrange, ylim=spatCore$yrange) +
  guides(fill=guide_legend(title="Days@Sea")) +
...
```{r spatial-extent-dominant-gears3, results='asis',fig.width = 5.5, fig.height = 9}
ggplot(polLand,aes(long,lat)) +
  geom_polygon(aes(group=group)) +
  coord_fixed(ratio=lonLatRatio(mean(spatCore$xrange),mean(spatCore$yrange)),xlim=spatCore$xrange, ylim=spatCore$yrange) +
  guides(fill=guide_legend(title="Days@Sea")) +
...
```{r , results='asis',fig.width = 5.5, fig.height = 9}
polLand         <- fortify(eurPols)
spatCore        <- list(xrange=quantile(ICES_VE$SI_LONG,probs=c(0.025,0.975)),
                        yrange=quantile(ICES_VE$SI_LATI,probs=c(0.025,0.975)))
polLand         <- subset(polLand,long >= spatCore$xrange[1] & long <= spatCore$xrange[2] &
